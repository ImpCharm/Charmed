label slimevillage_inn:
    ...
    menu:
        "Rest":
            call slimevillage_inn_rest from _call_slimevillage_inn_rest 
...
label slimevillage_inn_dead:
    if(game.slimeInnUnlocked == False):
        ...
        call slimevillage_inn_rest from _call_slimevillage_inn_rest_1
        jump slimevillage_inn
...
label slimevillage_inn_rest:
    ...
    call restoreAll from _call_restoreAll_5
    call dayPass() from _call_dayPass
    "You go to sleep for today."
    $ game.dayTime = time_midnight
    $ game.dayTime = round(game.dayTime)
    pause 3
    call setAmbience from _call_setAmbience
    return